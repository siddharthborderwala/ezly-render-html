...
const generateHead = ({username, description, font, background, image}: MetaBlock) => `
<head>
  <meta charset="UTF-8">
...
type generateHeaderArg = Pick<MetaBlock, 'username' | 'image'> & { socials: SocialBlock[] };
...
<header>
  ...
  <nav class="socials-list">
    ...
    `.trim()).reduce((acc, cur) => acc = acc + cur, '')}
  </nav>
</header>
`).trim();
...
<body>
  <div class="container">
    ${generateHeader({ username: meta.username, image: meta.image, socials: socials })}
